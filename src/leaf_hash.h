...
/**
 * Hash value and combine it with an already existing hash for generally for pointers
 */
#define MY_HASH_COMB_PTR(val, data) ( (val) ^ (my_hash((uint64_t)data) + 0x9e3779b9 + ((val)<<6) + ((val)>>2)) )
