            ...
            *bits_to_measure = my_malloc(n * sizeof(int));
            ...
            if (strcmp(cmd, "for") == 0) {
                ...
                *is_measure = true;
                ...
                uint32_t qt = get_q_num(in);
                (opt_symb && is_loop)? error_exit("Gate 'rx(pi/2)' does not support symbolic simulation.\n") : gate_rx_pihalf(circ, qt); //TODO:
            }
            else if (strcasecmp(cmd, "ry(pi/2)") == 0) {
                uint32_t qt = get_q_num(in);
                (opt_symb && is_loop)? error_exit("Gate 'ry(pi/2)' does not support symbolic simulation.\n") : gate_ry_pihalf(circ, qt); //TODO:
            }
